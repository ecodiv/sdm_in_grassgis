...
You can reduce the number of presence points using the `v.extract` module. If you also use the spatial thinning option ([paragraph -@sec-spatialthinning]), it is advisable to first carry out spatial thinning mannually (instead of doing this when running `v.maxent.swd`), as this will already reduce the number of points. If the number of points are still too many, use the `v.extract` module.

The steps below assume you have carried out the steps up to @exm-Xj32c2jubU in @sec-pointdensities. Check your region settings of the current mapset using `g.region`. The following steps with [v.to.rast](https://grass.osgeo.org/grass-stable/manuals/v.to.rast.html) and [r.to.vect](https://grass.osgeo.org/grass-stable/manuals/r.to.vect.html) will reduce the point density to one presence point per grid cell as explained in [paragraph -@sec-spatialthinning].
...
Open the `v.extract` and run it with the following settings:
...
Use the `g.remove` module to remove the intermediate layer.
