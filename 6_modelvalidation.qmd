...
While this approach is straightforward, the drawback is that the results can vary depending on which 20% of the points are selected for testing. A more robust and commonly used technique for estimating the accuracy of a predictive model is [cross-validation](https://en.wikipedia.org/wiki/Cross-validation_(statistics)). There are different types of cross-validation, but we'll focus on k-fold cross-validation here. In k-fold cross-validation, the original sample is randomly partitioned into **k** subsamples with an approximately equal number of records, as further explained in @fig-vkgSx6qmBy.

![Schematic explanation of a 4-fold cross-validation. The presence data is randomly partitioned into 4 groups with an approximately equal number of records. One group is set aside as the validation data for testing the model, while the remaining subsamples are combined to train the model. The model is then used to predict the presence of the locations of the validation subset. The results of the prediction are then compared with the known presence and background designations in the validation subset. This process is repeated as many times as there are subsamples, whereby each of the subsamples is used once as the validation data.](images/4-fold_cross-validation.svg){#fig-vkgSx6qmBy fig-align="left" width="600"}

Using k-fold cross-validation will give you a more reliable estimate of model accuracy, as it reduces the variability associated with a single random data split and leverages the entire dataset for both training and validation.

### Model training {#sec-611}

For this tutorial, we carry out a 4-fold cross-validation. To perform the cross-validation, we run `r.maxent.train` with the parameter [replicatetype]{.style-parameter} = [crossvalidate]{.style-value} and the parameter [replicates]{.style-parameter} = [4]{.style-value}. This instructs Maxent to perform a 4-fold cross-validation. As a result, Maxent trains four sub-models in the background, computes validation statistics for each sub-model, and then derives average statistics across the four sub-models.

::: {.callout-note appearance="simple"}
In cross-validation, multiple models are trained on slightly different subsets of the data. Technically, these are separate models because each one is fitted to a different data split and may therefore produce slightly different predictions. This variation reflects the inherent variability in the data.

Conceptually, however, these models are not independent in the same way as models trained with different algorithms. Instead, they represent repeated iterations of the same underlying model, used to assess robustness and generalizability. For simplicity, we therefore refer to these as "sub-models".
:::

Combining the cross-validation option with the [-y]{.style-parameter} will produce two new point layers: a sample prediction point layer and a background prediction point layer. In contrast to a single run (like in @exm-g8jUY2JKvW), both attribute tables are extended with two additional columns: the average predicted probability and the range of predicted probabilities across the four sub-models[^aophgf].

[^aophgf]: This functionality requires the [v.db.pyupdate](https://grass.osgeo.org/grass-stable/manuals/v.db.update.html) addon to be installed.

To keep the outputs clearly identifiable, we use the [samplepredictions]{.style-parameter} and [backgroundprediction]{.style-parameter} parameters to explicitly name the point layers containing predicted suitability scores. The sample prediction points are saved as [model02_suitability_samplepoints]{.style-value}, and the background prediction points as [model02_suitability_backgroundpoints]{.style-value}[^vpa].

[^vpa]: If these parameters are not provided, default names are assigned.

We use the [projectionlayers]{.style-parameter} parameter to specify the folder containing the environmental raster layers previously exported with `r.maxent.swd`. This will result in the creation of several raster prediction layers. These rasters summarize predictions across the four sub-models and represent the average, minimum, maximum, median, and standard deviation of predicted suitability. Using the [predictionlayer]{.style-parameter} parameter, we define [model02_suitability]{.style-value} as the root name for these outputs. The resulting raster layers share this base name and are distinguished by the suffixes [_av]{.style-output}, [_min]{.style-output}, [_max]{.style-output}, [_median]{.style-output}, and [_stddev]{.style-output}.
...
We want to store the result in the [Erebia_alberganus]{.style-data} mapset. Make sure you are working in that mapset, and that the region settings are correct.

``` bash
# Setup working directory and folder to store outputs
cd replace-for-path-to-working-directory
mkdir model_02

# Train
r.maxent.train samplesfile=model_input/swd01/species.swd environmentallayersfile=model_input/swd01/background_points.swd projectionlayers=model_input/ascii01 outputdirectory=model_02 replicatetype=crossvalidate samplepredictions=model02_suitability_samplepoints backgroundpredictions=model02_suitability_backgroundpoints predictionlayer=model02_suitability replicates=4 threads=4 memory=1000 -ygb
```
...
gs.run_command(
    ...
    samplepredictions="model02_suitability_samplepoints",
    backgroundpredictions="model02_suitability_backgroundpoints",
    predictionlayer="model02_suitability",
    replicates=4,
    threads=4,
    memory=1000,
    flags="ybg",
...
Create the folder [model_02]{.style-data} in your working directory using your favorite file manager/explorer. Next, create a new mapset and switch to this mapset using the Data panel. Alternatively, open the `g.mapsets` dialog and run it with the following parameter settings:

Next, open the `r.maxent.train` dialog and run the module with the following parameter settings:
...
To examine how well the four sub-models and the model overall performed, we take a look at the model statistics. We can find these statistics in the HTML file [Erebia_alberganus_obs.html]{.style-file} located in the [model_02]{.style-data} output folder. The page provides a summary of the results from the 4-fold cross-validation and includes links to the results of the individual sub-models in the top right corner. The average test AUC across the replicate runs is 0.9250, with a standard deviation of 0.002.
...
![ROC curve and the area under the curve statistics for model_02.](share/model_02/plots/occurrences_roc.png){#fig-rocaucmodel3 fig-align="left" group="corcom"}
...
![The omission/commission graph for model_02.](share/model_02/plots/occurrences_omission.png){#fig-omissionmodel3 fig-align="left" group="corcom"}
:::
...
::: {#exr-SErwbZwytm}
You would expect the training AUC to be higher than the test AUC. In some cases, however, the test AUC can higher than training AUC. Can you give a possible explanation?
:::
...
The default color scheme of the [Erebia_alberganus_obs_samplePredictions]{.style-data} represents the average predicted probabilities, based on the four sub-models (@fig-samlepredmodel03a). To visualizing the variability in predictions across the four sub-models, we use the values in the [Cloglog_range]{.style-data} column of the attribute table. These values represent the range (difference between the maximum and minimum predicted probabilities) across the four sub-models (@fig-samlepredmodel03b).
...
``` bash
v.colors map=model02_suitability_samplepoints use=attr column=Cloglog_range color=bcyr
```
...
gs.run_command(
    "v.colors",
    map="model02_suitability_samplepoints",
    use="attr",
...
![The [model02_suitability_samplepoints]{.style-data} vector layer with the GBIF occurrences. The colors represent the predicted probability that the species occurs at these locations, averaged over the four sub-models.](images/Erebia_alberganus_obs_samplePredictions1.png){#fig-samlepredmodel03a group="ytHc07Sg8P"}

## Range sample predictions

![The [model02_suitability_samplepoints]{.style-data} vector layer with GBIF occurrence data. The colors represent the range of predicted probabilities (the difference between the maximum and minimum values) across the four sub-models.](images/Erebia_alberganus_obs_samplePredictions2.png){#fig-samlepredmodel03b group="ytHc07Sg8P"}

## Mean vs range

![Density plot, with the predicted mean probabilities plotted against the range of predicted probabilities across the four iterations of model 3. The grey lines show the median mean and range values. See @exm-972VUKNJIx below for the code used to create this plot.](images/scatterplot_mean_range.png){#fig-meanvsrange fig-align="left"}
:::
...
v.scatterplot map=model02_suitability_samplepoints x=Cloglog_mean y=Cloglog_range type=density quadrants=median
```
...
v.extract input=model02_suitability_samplepoints where='"Cloglog_mean" < 0.3 AND "Cloglog_range" >= 0.09\' output=outliers
```
...
gs.run_command(
    "v.extract",
    input="model02_suitability_samplepoints",
...
To get a better idea about the spatial patterns of agreement and disagreement among the four sub-models outside the areas where the species was observed, we can examine the prediction raster layers with the average and standard deviation of the values generated by the four sub-models.

::: {.panel-tabset .exercise}
## Average predictions

![The [model02_suitability_avg]{.style-data} raster layer. The colors represent the predicted probability, averaged over the four sub-models.](images/Erebia_alberganus_obs_envdat_avg.png){#fig-predlaymodel03a group="ytHc07Sg8P"}

## Standard deviation of predictions

![The [model02_suitability_stddev]{.style-data} raster layer. The colors represent the standard deviation of predicted probabilities across the four sub-models.](images/Erebia_alberganus_obs_envdat_stddev.png){#fig-predlaymodel03b group="ytHc07Sg8P"}
:::

Besides the prediction layers with the average and standard deviation of the four sub-model, maps with the median, maximum and minimum values are created. It can be useful to examine these as well.

### Response curves {#sec-4responsecurves_model3}

The response curves in the HTML file [occurrences.html]{.style-file} in the [model_02]{.style-file} folder show the mean response of the 4 sub-models (red) and and the mean +/- one standard deviation (blue, two shades for categorical variables).

::::: {.panel-tabset .exercise}
## Marginal response curves

::: {#fig-QB4dnXf8U3 layout-ncol="4"}
![](share/model_02/plots/occurrences_bio_1.png){group="QB4dnXf8U3a"}

![](share/model_02/plots/occurrences_bio_2.png){group="QB4dnXf8U3a"}

![](share/model_02/plots/occurrences_bio_4.png){group="QB4dnXf8U3a"}

![](share/model_02/plots/occurrences_bio_8.png){group="QB4dnXf8U3a"}

![](share/model_02/plots/occurrences_bio_9.png){group="QB4dnXf8U3a"}

![](share/model_02/plots/occurrences_bio_13.png){group="QB4dnXf8U3a"}

![](share/model_02/plots/occurrences_bio_14.png){group="QB4dnXf8U3a"}

![](share/model_02/plots/occurrences_bio_15.png){group="QB4dnXf8U3a"}
...
![](share/model_02/plots/occurrences_bio_1_only.png){group="nCjTprfaAra"}

![](share/model_02/plots/occurrences_bio_2_only.png){group="nCjTprfaAra"}

![](share/model_02/plots/occurrences_bio_4_only.png){group="nCjTprfaAra"}

![](share/model_02/plots/occurrences_bio_8_only.png){group="nCjTprfaAra"}

![](share/model_02/plots/occurrences_bio_9_only.png){group="nCjTprfaAra"}

![](share/model_02/plots/occurrences_bio_13_only.png){group="nCjTprfaAra"}

![](share/model_02/plots/occurrences_bio_14_only.png){group="nCjTprfaAra"}

![](share/model_02/plots/occurrences_bio_15_only.png){group="nCjTprfaAra"}

Response curves created by running a model based on only the specific variable as explanatory variable.
:::
:::::

The marginal response curves reveal substantial differences in stability across predictors. For some variables, the mean response curve is accompanied by a wide standard deviation across cross-validated sub-models (visible as a broad blue band), indicating high variability in the estimated effect. This variability reflects greater uncertainty in the influence of these predictors and, consequently, increased uncertainty in model predictions for locations where predictor values fall within ranges showing large dispersion among sub-models.

For certain predictors, the direction of the marginal response curve differs from that of the corresponding single-variable response curve (e.g., bio14). Such discrepancies arise when predictors interact within the multivariate model. Marginal response curves reflect the combined effects of a focal variable while accounting for the presence of other predictors, whereas single-variable response curves are fitted in isolation and ignore interactions. As a result, interactions among predictors can lead to marginal responses that differ in shape or even direction from those suggested by single-variable responses.

that several single-variable response curves exhibit pronounced oscillations, characterized by multiple peaks and troughs. These patterns are unlikely to represent biologically meaningful responses and instead suggest that the model is fitting noise rather than signal. Such behavior is commonly associated with overfitting, which may result from excessive model complexity, limited sample size, or strong collinearity among predictors. In these cases, response curves should be interpreted with caution, as they may not reflect robust or generalizable ecological relationships.
...
# It should already be the correct region, but
# just to be sure
g.mapset mapset=Erebia_alberganus
g.region raster=bio_1@climate

# Combine the occurrence and background points
v.patch input=model01_suitability_samplepoints,model01_suitability_backgroundpoints output=referencepoints

# Convert the point layer to a raster layer
...
gs.run_command(
    "v.patch",
    input=["model01_suitability_samplepoints", "model01_suitability_backgroundpoints"],
...
Convert the point layer to a raster with the `v.to.rast` module. Open it and run it with the following parameters:
...
We use the [ref_rast]{.style-parameter} parameter to set the reference raster layer, which we create first, as the reference layer.

Alternatively, we could have used the [ref_vect]{.style-parameter} parameter with the [referencepoints]{.style-data} vector layer directly. If you do so, be aware that the vector layer contains multiple point within some raster cells. The `r.mess` module will ignore all but one with a warning message.

By first converting the point layer to a raster, and using that as input, we eliminate duplicate points up front, and thereby avoid the warning message.
...
# Compute mess
<<<<<<< HEAD
r.mess -m -n -k -c -i ref_env=bio_1,bio_2,bio_4,bio_8,bio_9,bio_13,bio_14,bio_15,bio_19 proj_env=ECE3V_585_8100.1,ECE3V_585_8100.2,ECE3V_585_8100.4,ECE3V_585_8100.8,ECE3V_585_8100.9,ECE3V_585_8100.13,ECE3V_585_8100.14,ECE3V_585_8100.15,ECE3V_585_8100.19 ref_rast=referencepoints output=mess nprocs=2 memory=1000
=======
...
Check the [manual page](https://grass.osgeo.org/grass-stable/manuals/addons/r.mess.html) about the meaning of the flags [m]{.style-parameter}, [n]{.style-parameter}, [k]{.style-parameter}, [c]{.style-parameter} and [i]{.style-parameter}.

## {{< fa brands python >}}

``` python
gs.run_command("r.mess", flags="mnkci",
    ref_env=["bio_1", "bio_2", "bio_4", "bio_8", "bio_9",
             "bio_13", "bio_14", "bio_15", "bio_19"],
    proj_env=["ECE3V_585_8100.1", "ECE3V_585_8100.2", "ECE3V_585_8100.4",
              "ECE3V_585_8100.8", "ECE3V_585_8100.9", "ECE3V_585_8100.13",
              "ECE3V_585_8100.14", "ECE3V_585_8100.15", "ECE3V_585_8100.19"],
...
|----|----|
| ref_env | bio_1,bio_2,bio_4,bio_8,bio_9,bio_13,bio_14,bio_15,bio_19 |
| proj_env | ECE3V_585_8100.1,ECE3V_585_8100.2,ECE3V_585_8100.4,ECE3V_585_8100.8,ECE3V_585_8100.9,ECE3V_585_8100.13,ECE3V_585_8100.14,ECE3V_585_8100.15,ECE3V_585_8100.19 |
| proj_env | attr |
...
The spatial extent and resolution of the rasters produced by `r.mess` are inherited from the input rasters specified via the [proj_env]{.style-parameter} parameter. If you followed the workflow step by step, this should already be correctly configured.

However, if you imported full future climate layers (e.g. global coverage) but only want to compute r.mess outputs for a smaller study area, proceed as follows:

1. Set the computational region to your study area and save it using `g.region save=name-of-region`. 
2. When running `r.mess`, specify [proj_region]{.style-parameter} = [name-of-region]{.style-value} to restrict the output rasters to that saved region.

See the [manual page](https://grass.osgeo.org/grass-stable/manuals/addons/r.mess.html) for more details.
