...
To confirm that the resulting map [E_alberganus_probability2]{.style-data} is identical to the map [E_alberganus_probability]{.style-data} that we created earlier in @sec-4trainthemodel, we compute the difference between the two maps using the r.mapcalc` module.
...
1.  With `echo -e` you normally print a text to the screen. Here, these are the color instructions. The '\|' (pipe) symbol, takes what would have been printed on the screen and sends it directly into the next command, `r.colors`, as input. The [rules=-]{.style-parameter} part tells `r.colors` to get its color instructions from the previous command (instead of a file). The dash (-) means "read from the pipeline," which is the output of echo.
...
2.  You can choose in what unit(s) `r.report` should report the area statistics.
...
[^5_model_prediction-7]: You can choose in what unit(s) `r.report` should report the area statistics.
